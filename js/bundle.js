...
function calc() {
  ...
  calcTotal(); // fun которая при обновлении стр инициализирует класс активности при нуобходимых данных из localStorage
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // fun которая берет статические значения
    ...
    element.forEach(elem => {
      elem.addEventListener('click', e => {
        if (e.target.getAttribute('data-ratio')) {
          ratio = +e.target.getAttribute('data-ratio'); //  при перезагрузке localStorage запомнит данные пользователя

          localStorage.setItem('ratio', +e.target.getAttribute('data-ratio'));
        } else {
          sex = e.target.getAttribute('id'); //  при перезагрузке localStorage запомнит данные пользователя                
  ...
  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); // fun которая берет значения из инпутов
...
function cards() {
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResurce)('http://localhost:3000/menu').then(data => {
    data.forEach(_ref => {
      let {
        img,
        altimg,
...
function forms(formSelector, modalTimerId) {
  ...
  const message = {
    // объект для вывода текста при отправке формы
  ...
  function showThanksModal(message) {
    ...
    thanksModal.innerHTML = `
        ...
        `;
    document.querySelector('.modal').append(thanksModal); // если пользователь захочет через какоето время открыть модалку, то он снова должен видеть форму
...
function showModal(modalSelector, modalTimerId) {
  ...
  modal.classList.remove('hide'); // modal.classList.toggle('show'); // необходимо из верстки убрать класс hide
  ...
  if (modalTimerId) {
    // очитска таймера будет, только если есть modalTimerId
...
function modal(modalSelector, triggerSelector, modalTimerId) {
  // modalTimerId передаем везде где вызывается showModal
  // Modal
  const modal = document.querySelector(modalSelector),
  ...
  modal.addEventListener('click', e => {
    if (e.target === modal || e.target.getAttribute('data-close') == "") {
      // если объект клика это modal, а не modal__dialog, то закрываем окно || если getAttribute(true or false) вернул пустую строку то тоже закрываем окно
      closeModal(modalSelector);
    }
  });
  document.addEventListener('keydown', e => {
    if (e.code === 'Escape' && modal.classList.contains('show')) {
      //  проверяется код события
      closeModal(modalSelector);
    }
  ...
  function showModalByScroll() {
    if (window.scrollY + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
      // если сумма того на сколько пролистана страница вниз совпадает с высотой элемента >= высоты элемента с прокруткой
...
function slider(_ref) {
  ...
  const slider = document.querySelector(container),
        ...
        width = window.getComputedStyle(slidesWrapper).width; // Slider 2
  ...
  slides.forEach(slide => {
    // устанвливаем для каждого слайда единую ширину, потому что они мб разные
  ...
  if (slides.length < 10) {
    ...
    total.textContent = slides.length; // current.textContent = slideIndex;
  }
...
const postData = async (url, data) => {
  // url который приходит, а data это данные которые будут поститься // 1) функция настривает наш запрос // async говорит что внутри функции будет какойто аснихронный код
  const result = await fetch(url, {
    // fetch это наш запрос // 2) посылет этот запрос на сервер // await ставим перед теми операциями, которые необходимо дождаться
    method: 'POST',
    //каким образом
    headers: {
      // если formData то headers не используем
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_5__.showModal)('.modal', modalTimerId), 600000);
  ...
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_5__["default"])('.modal', '[data-modal]', modalTimerId);
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2022-06-29');
});
})();
